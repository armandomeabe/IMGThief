...
namespace TesseractBasedSolution
{
    class Program
    {
        static void Main(string[] args)
        {
            // Code usage sample
            Ocr ocr = new Ocr();
            using (Bitmap bmp = new Bitmap(@"E:\temp\imgthief\-308010_1205177144.jpg"))
            {
                tessnet2.Tesseract tessocr = new tessnet2.Tesseract();
                tessocr.Init(null, "eng", false);
                tessocr.GetThresholdedImage(bmp, Rectangle.Empty).Save("E:\\temp\\" + Guid.NewGuid().ToString() + ".bmp");
